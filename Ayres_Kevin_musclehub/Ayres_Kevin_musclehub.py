...
unpivoted = just_apps.groupby(['ab_test_group', 'is_member']).first_name.count().reset_index()
member_pivot = unpivoted.pivot(columns='is_member',
                              index='ab_test_group',
                              values='first_name').reset_index()
...
unpivoted2 = df.groupby(['ab_test_group', 'is_member']).first_name.count().reset_index()
final_member_pivot = unpivoted2.pivot(columns='is_member',
                                     index='ab_test_group',
                                     values='first_name').reset_index()
...
ax.set_yticks([0.00, 0.02, 0.04, 0.06, 0.08, 0.10, 0.12, 0.14])
